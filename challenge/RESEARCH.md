...
2. strictly exceeds all three current champion component thresholds in one fixed seed-0 full-fidelity execution;
3. has the same improvement direction on at least two rolling train cutoffs;
4. reports wall time, API tokens/cost, failures, and manual interventions; and
5. is regenerated by a frozen-prompt clean campaign with zero manual interventions before final designation.
